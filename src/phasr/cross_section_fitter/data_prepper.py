...
def import_dataset(path:str,save_name:str,Z:int,A:int,correlation_stat_uncertainty=None,correlation_syst_uncertainty=None,fit_luminosity='n',**args):
    ...
    if fit_luminosity == 'y':
        ...
        rate_units = input("In what units is the cross section*luminosity (1/s,1/ms,1/micros)?")
        if rate_units=="1/s":
            cross_section_scale=1
        elif rate_units=="1/ms":
            cross_section_scale=1e+3
        elif rate_units=="1/micros":
            cross_section_scale=1e+6
        else:
            cross_section_scale=float(input("Unknown unit for cross section*luminosity."))
        
        cross_section_data = cross_section_dataset_input[:,(rate_col,)]*cross_section_scale
        ...
        if cross_section_or_fraction=="direct":
            ...
            raise ValueError("input is either direct or relative.")
    else:
        raise ValueError("input is either y or n.")
