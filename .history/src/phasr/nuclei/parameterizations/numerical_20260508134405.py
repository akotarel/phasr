        ...
        for L in np.arange(0,2*self.spin+1,dtype=int):
            ...
            for multipole in multipoles:
                if hasattr(self,'F'+multipole):
                    ...
                    rrange_m2=[0,40.,0.05]
                    rhom2=fourier_transform_mom_to_pos(FF,multipole+'m2'+'_'+self.name,self.qrange,rrange_m2,L=L,norm=1,extra_pow=extra_pow-2,renew=self.renew)
...
def fourier_transform_mom_to_pos(fct_q,name,qrange,rrange,L=0,norm=1,extra_pow=0,renew=False,extrapolation_type="exp"):
    ...
    if extrapolation_type == "exp":
        ...
        fct_r = partial(field_ultimate_inverse,R=r_crit,field_spl=fct_r_spl) # Asymptotic: inverse function
